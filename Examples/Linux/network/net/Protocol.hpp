...
inline uint32_t hostToNet32(uint32_t v) {
    ...
    uint32_t out; std::memcpy(&out, b, 4); return out;
}
...
inline uint64_t hostToNet64(uint64_t v) {
    uint64_t r = 0;
    for (int i=0;i<8;++i) r |= ((v >> (56 - 8*i)) & 0xFFull) << (8*(7-i));
    return r;
}
...
inline ResourceReportPayload decodeResourceReport(const std::vector<uint8_t>& buf) {
    if (buf.size() != 12) throw std::runtime_error("Bad ResourceReport size");
    ResourceReportPayload p{};
    uint64_t ramN; uint32_t thrN;
