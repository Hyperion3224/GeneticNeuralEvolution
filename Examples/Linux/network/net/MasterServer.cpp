...
bool MasterServer::setupListener() {
    ...
    if (listener_ == INVALID_SOCKET) { LOG_ERROR("socket() failed"); return false; }
    ...
    if (::bind(listener_, (sockaddr*)&addr, sizeof(addr)) != 0) {
        ...
        ::close(listener_); listener_ = INVALID_SOCKET; return false;
    }
...
void MasterServer::stop() {
    ...
    for (auto& [id, info] : registry_.snapshot()) {
        (void)id; (void)info; // actual Connection instances are owned per-connection loops
...
void MasterServer::acceptLoop() {
    while (!stopping_.load()) {
        sockaddr_in peerAddr{}; socklen_t len = sizeof(peerAddr);

        socket_t s = ::accept(listener_, (sockaddr*)&peerAddr, &len);
        if (s == INVALID_SOCKET) {
            if (!stopping_.load()) LOG_WARN("accept() failed or interrupted");
            break;
        }
        ...
        NodeInfo info; info.ip = ip; info.alive = true; info.lastSeen = std::chrono::steady_clock::now();
