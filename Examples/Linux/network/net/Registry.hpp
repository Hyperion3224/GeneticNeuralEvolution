...
public:
    ...
    bool insert(NodeId id, const NodeInfo& info) {
        ...
        if (nodes_.size() >= maxNodes_) return false;
        nodes_[id] = info;
        return true;
    }
    ...
    void update(NodeId id, const std::function<void(NodeInfo&)>& fn) {
        std::lock_guard<std::mutex> lk(mu_);
        auto it = nodes_.find(id);
        if (it != nodes_.end()) fn(it->second);
    }

    std::optional<NodeInfo> get(NodeId id) const {
        std::lock_guard<std::mutex> lk(mu_);
        if (auto it = nodes_.find(id); it != nodes_.end()) return it->second;
        return std::nullopt;
    }
    ...
    std::vector<std::pair<NodeId, NodeInfo>> snapshot() const {
        ...
        for (auto& kv : nodes_) out.push_back(kv);
        return out;
    }
